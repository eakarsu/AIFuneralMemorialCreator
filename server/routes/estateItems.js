...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const countResult = await pool.query('SELECT COUNT(*) FROM estate_items WHERE user_id = $1', [req.user.id]);
    const total = parseInt(countResult.rows[0].count);
    const result = await pool.query('SELECT * FROM estate_items WHERE user_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3', [req.user.id, limit, offset]);
